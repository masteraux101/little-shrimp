...
const messages = {
  en: {
    ...
    slashScheduleDesc: 'Create a cron scheduled task from conversation code', slashGithubStatusDesc: 'List workflows and active runs in the repo', slashGithubRunDesc: 'Trigger (workflow_dispatch) a specific workflow', slashGithubDeleteDesc: 'Delete a workflow file', slashLoopDesc: 'Deploy a persistent loop agent via GitHub Actions', slashLoopStatusDesc: 'Check status of running loop agents', slashLoopConnectDesc: 'Connect to a running loop agent and chat directly', slashLoopDisconnectDesc: 'Disconnect from the current loop agent', slashSkillsDesc: 'Manage skills — browse built-in library & custom URLs', slashSoulDesc: 'Show current SOUL name and URL', slashSoulListDesc: 'Browse available built-in SOULs', slashCompactDesc: 'Compress conversation history into a summary', slashClearDesc: 'Clear all messages in current session',
  ...
  zh: {
    ...
    slashScheduleDesc: '从对话代码创建 cron 定时任务', slashGithubStatusDesc: '查看仓库中的工作流和运行状态', slashGithubRunDesc: '触发指定 workflow（workflow_dispatch）', slashGithubDeleteDesc: '删除 workflow 文件', slashLoopDesc: '通过 GitHub Actions 部署持久循环代理', slashLoopStatusDesc: '检查运行中的循环代理状态', slashLoopConnectDesc: '连接到运行中的循环代理并直接对话', slashLoopDisconnectDesc: '断开当前循环代理连接', slashSkillsDesc: '管理技能：浏览内置库与自定义 URL', slashSoulDesc: '查看当前 SOUL 名称与 URL', slashSoulListDesc: '浏览可用内置 SOUL', slashCompactDesc: '将会话历史压缩为摘要', slashClearDesc: '清空当前会话所有消息',
